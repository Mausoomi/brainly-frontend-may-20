import { UpdateScienceFiction } from "../../../store/Actions/adminActions";
import {
...
const AdminEDITScienceFiction = () => {
  ...
  useEffect(() => {
    dispatch(fetchScienceDataByID(id));
  }, [dispatch, id]);
  ...
  const HandleSubmit = (e) => {
    e.preventDefault();
    dispatch(UpdateScienceFiction({ formData, id }));
    setFormData({
      Title: "",
      Image: [],
      Status: "",
    ...
    navigate("/Admin/Admin-ScienceFiction");
  };
  {
  ...
  return (
    ...
    <div className="w-full h-full rounded-start-5 p-2 overflow-y-auto">
      <form className="w-full mt-3 p-2" method="post" onSubmit={HandleSubmit}>
        ...
        <div className="flex justify-between w-full bg-gray-200 mt-2">
          <input
            type="text"
        ...
        {/* Rendering Storyadvenure fields */}
        {formData?.Storyadvenure?.content?.map((storyadvenure, index) => (
          <div className="mt-2" key={index}>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2 md:gap-0">
              {storyadvenure?.Storyimage?.map((md, imgIndex) => (
            ...
            {/* Rendering Paragraph fields */}
            {storyadvenure?.Paragraph?.map((paragraph, pIndex) => (
              <div
                className="flex justify-between w-full bg-gray-200 mt-2"
                key={pIndex}
              >
                <input
                  type="text"
            ...
            {/* Delete button for removing the Storyadvenure field */}
            {index !== 0 && (
              <button
        ...
        {formData?.Brainquest?.map((brainquest, index) => (
          <div className="mt-2" key={index}>
            ...
            <input
              ...
              required
            />
            {/* Delete button for removing the Brainquest field */}
            {index !== 0 && (
              <button
                className="bg-blue-500 hover:bg-blue-400 text-white font-bold py-2 px-4 border-b-4 border-blue-700 hover:border-blue-500 initialpadding"
                onClick={() => handleRemoveBrainquest(index)}
...
export default AdminEDITScienceFiction;
