...
import { imageUpload } from "../../../store/Actions/adminActions";
import { UpdateSportification } from "../../../store/Actions/adminActions";
import { fetchSportDataByID } from "../../../store/Actions/storyActions";

const AdminEDITSportification = () => {
  ...
  useEffect(() => {
    dispatch(fetchSportDataByID(id));
  }, [dispatch, id]);
  ...
  const HandleSubmit = (e) => {
    e.preventDefault();
    dispatch(UpdateSportification({ formData, id }));
    setFormData({
      Title: "",
      Image: [],
      Status: "",
  ...
  return (
    ...
    <div className="w-full h-full rounded-start-5 p-2 overflow-y-auto">
      <form className="w-full mt-3 p-2" method="post" onSubmit={HandleSubmit}>
        ...
        {/* Image input Links */}
        {formData?.Image?.length < 10 && (
          <div className="w-full sm:w-1/2 lg:w-1/4 mt-2 mb-2">
            <div className="card-body">
              <p className="text-xs">
                Select image file to upload on the cover of the fanstasy.
              </p>
        ...
        {formData?.Wordexplore?.map((wordexplore, index) => (
          <div className="mt-2" key={index}>
            ...
            <div className="flex w-full bg-gray-200 justify-between mt-2">
              ...
              {index !== 0 && (
                <button
                  className="bg-blue-500 hover:bg-blue-400 text-white font-bold py-2 px-4 border-b-4 border-blue-700 hover:border-blue-500 px-4 py-2"
                  onClick={() => handleRemoveWordexplore(index)}
                >
                  Delete Wordexplore
                </button>
            ...
            {index === formData?.Wordexplore?.length - 1 && (
              <button
                className="bg-blue-500 hover:bg-blue-400 text-white font-bold py-2 px-4 border-b-4 border-blue-700 hover:border-blue-500 mt-2 px-4 py-2"
                onClick={handleAddWordexplore}
              >
                Add More Wordexplore
              </button>
...
export default AdminEDITSportification;
