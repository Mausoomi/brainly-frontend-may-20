...
import { fetchFantasyDataByID } from "../../../store/Actions/storyActions";
import {
  UpdateFantasy,
  imageUpload,
} from "../../../store/Actions/adminActions";

const AdminEDITFantasy = () => {
  ...
  useEffect(() => {
    dispatch(fetchFantasyDataByID(id));
  }, [dispatch, id]);
  ...
  useEffect(() => {
    if (data) {
      setFormData({
        ...
        Status: data.Status,
        Wordexplore: data.Wordexplore,
        Storyadvenure: {
          Storytitle: data?.Storyadvenure?.Storytitle,
          content: data?.Storyadvenure?.content?.map((content) => ({
            Storyimage: [...content.Storyimage],
            Paragraph: [...content.Paragraph],
          })),
        },
        Brainquest: data.Brainquest?.map((brainquest) => ({
          Question: brainquest.Question,
          Option: [...brainquest.Option],
          Answer: brainquest.Answer,
        })),
      });
    }
  }, [data]);
  ...
  const HandleSubmit = (e) => {
    e.preventDefault();
    dispatch(UpdateFantasy({ formData, id }));
    setFormData({
      Title: "",
      Image: [],
      Status: "",
  ...
  return (
    // <div className=" h-100 rounded-start-5 p-2  overflowdiv">
    ...
    //           value={brainquest?.Question}
    //           onChange={(e) => handleChange(e, index)}
    //           required
    //         />
    //         <div className="d-flex w-100 bg-light justify-content-between mt-2">
    //           {formData?.Brainquest[index]?.Option?.map(
    //             (option, optionIndex) => (
    //               <input
    //                 type="text"
    //                 className="form-control w-25 "
    ...
    //       className="btn btn-outline-primary mt-2 initialpadding"
    //       onClick={() =>
    //         setFormData({ ...formData, Brainquest: [initialBrainquest] })
    //       }
    //     >
    //       Reset Brainquest
    //     </button>
    ...
    //   </form>
    // </div>
    <div className="w-full h-full rounded-start-5 p-2 overflow-y-auto">
      <form className="w-full mt-3 p-2" method="post" onSubmit={HandleSubmit}>
        <div className="flex flex-wrap">
          {formData?.Image?.map((md, index) => {
            return (
              <div
                className="w-full sm:w-1/2 lg:w-1/4 mt-2 md:mt-0 mb-2 md:mb-0"
                key={index}
              >
                <img
                  className="w-full"
                ...
                />
                <span
                  className="bg-blue-500 hover:bg-blue-400 text-white font-bold py-2 px-4 border-b-4 border-blue-700 hover:border-blue-500 mt-2 my-0.5 cursor-pointer"
                  onClick={() => handleImageRemoval(md, index, "Image")}
                >
                  Delete
                </span>
        ...
        {/* Image input Links */}
        {formData?.Image?.length < 10 && (
          <div className="w-full sm:w-1/2 lg:w-1/4 mt-2 md:mt-0 mb-2 md:mb-0">
            <div className="card-body">
              <p className="text-xs">
        ...
        <div className="flex w-full bg-gray-300 justify-between items-center mt-2">
          <input
            type="text"
            ...
            required
          />
          <div className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500l w-1/4">
            <label htmlFor="Status">Select Status</label>
            <select
              name="Status"
              value={formData?.Status}
        ...
        {formData?.Wordexplore?.map((wordexplore, index) => (
          <div className="mt-2" key={index}>
            ...
            <div className="flex w-full bg-gray-300 justify-between items-center mt-2">
              ...
              {index !== 0 && (
                <button
                  className="bg-blue-500 hover:bg-blue-400 text-white font-bold py-2 px-4 border-b-4 border-blue-700 hover:border-blue-500 initialpadding"
                  onClick={() => handleRemoveWordexplore(index)}
                >
                  Delete Wordexplore
                </button>
        ...
        {formData?.Storyadvenure?.content?.map((storyadvenure, index) => (
          <div className="mt-2" key={index}>
            <div className="flex flex-wrap">
              {storyadvenure?.Storyimage?.map((md, imgIndex) => (
                ...
                >
                  ...
                  />
                  <span
                    className="badge bg-red-500 rounded-full ml-1 cursor-pointer"
                    onClick={() =>
                      handleImageRemoval(
                        md,
                        index,
            ...
            {storyadvenure?.Paragraph?.map((paragraph, pIndex) => (
              <div
                className="flex w-full bg-gray-300 justify-between items-center mt-2"
                key={pIndex}
              >
                <input
                  type="text"
            ...
            {index !== 0 && (
              <button
                className="bg-blue-500 hover:bg-blue-400 text-white font-bold py-2 px-4 border-b-4 border-blue-700 hover:border-blue-500 initialpadding mt-2"
                onClick={() => handleRemoveStoryadvenure(index)}
              >
                Delete Wordexplore
              </button>
        ...
        <button
          type="submit"
          className="bg-blue-500 hover:bg-blue-400 text-white font-bold py-2 px-4 border-b-4 border-blue-700 hover:border-blue-500 mt-4"
        >
          Submit the Fantasy
        </button>
      </form>
...
export default AdminEDITFantasy;
