import { UpdateHistoryFiction } from "../../../store/Actions/adminActions";
...
const AdminEDITHistoryFiction = () => {
  ...
  const HandleSubmit = (e) => {
    e.preventDefault();
    dispatch(UpdateHistoryFiction({ formData, id }));
    setFormData({
      Title: "",
      Image: [],
      Status: "",
  ...
  return (
    ...
    <div className="w-full h-full rounded-start-5 p-2 overflow-y-auto">
      <form className="w-full mt-3 p-2" method="post" onSubmit={HandleSubmit}>
        <div className="Addteacherimage_box grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2 mt-2 md:mt-0 mb-2">
          {formData?.Image?.map((md, index) => {
            return (
              <div className="mt-2" key={index}>
                <img
...
export default AdminEDITHistoryFiction;
