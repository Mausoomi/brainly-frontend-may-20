...
const AdminEDITAdventure = () => {
  ...
  const HandleSubmit = (e) => {
    e.preventDefault();
    dispatch(UpdateAdventure({ formData, id }));
    setFormData({
      Title: "",
      Image: [],
      Status: "",
    ...
    navigate("/Admin/Admin-Adventure");
  };
  ...
  return (
    ...
    <div className="w-full h-full rounded-start-5 p-2 overflow-y-auto">
      <form className="w-full mt-3 p-2" method="post" onSubmit={HandleSubmit}>
        <div className="Addteacherimage_box grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-2 sm:gap-4">
          {formData?.Image?.map((md, index) => {
            return (
              <div className="mt-2" key={index}>
                <img
