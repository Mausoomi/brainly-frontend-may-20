...
import { fetchMysteryDataByID } from "../../../store/Actions/storyActions";

const AdminEDITMystery = () => {
  ...
  useEffect(() => {
    dispatch(fetchMysteryDataByID(id));
  }, [dispatch, id]);
  ...
  const HandleSubmit = (e) => {
    e.preventDefault();
    dispatch(UpdateMystery({ formData, id }));
    setFormData({
      Title: "",
      Image: [],
      Status: "",
  ...
  return (
    // <div className=" h-100 rounded-start-5 p-2  overflow-y-auto">
      ...
      <form className="w-full mt-3 p-2" method="post" onSubmit={HandleSubmit}>
        <div className="flex flex-wrap">
          {formData?.Image?.map((md, index) => {
            return (
              <div
                className="w-full sm:w-1/2 lg:w-1/4 mt-2 sm:mt-0 mb-2 sm:mb-0"
                key={index}
              >
                <img
        ...
        {/* Image input Links */}
        {formData?.Image?.length < 10 && (
          <div className="w-full sm:w-1/2 lg:w-1/4 mt-2 sm:mt-0 mb-2 sm:mb-0">
            <div className="card-body">
              <p className="text-xs">
        ...
        <div className="flex w-full bg-gray-300 justify-between mt-2">
          <input
            type="text"
        ...
        {formData?.Storyadvenure?.content?.map((storyadvenure, index) => (
          <div className="mt-2" key={index}>
            ...
            {storyadvenure?.Paragraph?.map((paragraph, pIndex) => (
              <div
                className="flex w-full bg-gray-300 justify-between mt-2"
                key={pIndex}
              >
                <input
                  type="text"
...
export default AdminEDITMystery;
